...
async fn test_sha256_authentication() {
    ...
    match result {
        Ok((client, connection)) => {
            ...
            // 测试数据库操作
            client.execute("CREATE TEMPORARY TABLE auth_test (id INT, name TEXT)", &[]).await.unwrap();
            client.execute("INSERT INTO auth_test VALUES (1, 'test')", &[]).await.unwrap();
...
async fn test_md5_sha256_authentication() {
    ...
    let connection_string = "host=localhost port=5433 user=gaussdb password=Gaussdb@123 dbname=postgres";
    ...
    match result {
        Ok((mut client, connection)) => {
            ...
            transaction.execute("CREATE TEMPORARY TABLE md5_test (data TEXT)", &[]).await.unwrap();
            transaction.execute("INSERT INTO md5_test VALUES ('md5_sha256_test')", &[]).await.unwrap();
            transaction.commit().await.unwrap();
            
            let rows = client.query("SELECT data FROM md5_test", &[]).await.unwrap();
    ...
    for (i, conn_str) in test_cases.iter().enumerate() {
        ...
        match result {
            Ok((client, connection)) => {
                ...
                // 简单验证
                let rows = client.query("SELECT 'connection_test' as result", &[]).await.unwrap();
...
async fn test_concurrent_connections() {
        ...
        let handle = tokio::spawn(async move {
            ...
            match result {
                Ok((client, connection)) => {
                    ...
                    // 执行查询
                    let rows = client.query("SELECT $1::INT as connection_id", &[&i]).await.unwrap();
    ...
    println!("✅ {}/3 concurrent connections successful", successful_connections);
    assert!(successful_connections >= 1, "At least one connection should succeed");
}
